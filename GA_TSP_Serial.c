...
char citiesNameFile[255]="inp100.txt"; //Stores all city names
...
void print_chromosome(Chromosome *ptr_chromosome){
    ...
    for (int i = 0; i < chromo_length; i++)
      {  
        ...
        //fprintf(op_file,"%d",ptr_chromosome->genes[i] );
        //fprintf(op_file, ",");
...
void sort_population(Chromosome *population){
    ...
    //at this point the population will be sorted
}
...
int main(){
    ...
    popl_size = 100;
    no_generation = 0;
    //parse_arguments('inp10.txt', 'inp10.txt');
    ...
    int  i = 0;
    Chromosome *ptr_chromosome;
    FILE *file;
    file= fopen("result100_time.txt", "w+");
    //FILE *file_2;
    //file_2= fopen("result100.txt", "w+");
   ...
   { 
       printf("Itr = %d \n",k);
       //printf("%s", ",  ");
       no_generation=no_generation+1000;
    ...
    sort_population(population); //Sort acc to fitness value
     ...
     while(i < no_generation){
      ...
      //print_chromosome(&population[0]);
       ptr_chromosome=&population[0];
        /*
      for (int j = 0; j < chromo_length; j++)
      {  
        //printf("%d_", ptr_chromosome->genes[j]);
        fprintf(file_2,"%d",ptr_chromosome->genes[j]);
        fprintf(file_2,"%s", ",");
      }

      fprintf(file,"%f",ptr_chromosome->fitness );
      fprintf(file,"%s", ",");
      fprintf(file_2,"%d\n");
        */
      i++;
